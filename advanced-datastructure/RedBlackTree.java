...
public class RedBlackTree<AnyType extends Comparable<? super AnyType>> {
    ...
    //remove a node from tree function
    public void remove(AnyType x) {
        if (contains(x)) {
            remove(x, header.right);
        }
    }

    private RedBlackNode<AnyType> remove(AnyType x, RedBlackNode<AnyType> t) {
        if (t != nullNode) {
            rotations++;
    ...
    private int height(RedBlackNode<AnyType> t) {
        if (t == nullNode)
            return -1;
        else
